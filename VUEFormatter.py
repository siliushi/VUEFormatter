...
RC_FILE = ".vuebeautifyrc"
SETTINGS_FILE = "VUEFormatter.sublime-settings"
KEYMAP_FILE = "Default ($PLATFORM).sublime-keymap"
OUTPUT_VALID = b"*** VUEFormatter output ***"

class VueformatterCommand(sublime_plugin.TextCommand):
  def run(self, edit):
...
class VueformatterEventListeners(sublime_plugin.EventListener):
  @staticmethod
  def on_pre_save(view):
    if PluginUtils.get_pref("format_on_save"):
      view.run_command("vueformatter")

class VueformatterSetFormatterPrefsCommand(sublime_plugin.TextCommand):
  def run(self, edit):
    PluginUtils.open_config_rc(self.view.window())

class VueformatterSetPluginOptionsCommand(sublime_plugin.TextCommand):
  def run(self, edit):
    PluginUtils.open_sublime_settings(self.view.window())

class VueformatterSetKeyboardShortcutsCommand(sublime_plugin.TextCommand):
  def run(self, edit):
...
class VueformatterSetNodePathCommand(sublime_plugin.TextCommand):
  def run(self, edit):
    PluginUtils.open_sublime_settings(self.view.window())
